...
def wsprlive_get_info(circuit, current_date):
    return wsprlive_get(
        f"SELECT tx_lat, tx_lon, rx_lat, rx_lon, power  FROM rx WHERE tx_sign = '{circuit['tx']}' AND rx_sign = '{circuit['rx']}' AND '{current_date}' <= time AND time < '{current_date + relativedelta(months=1)}' LIMIT 1")[
        0]
