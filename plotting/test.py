...
for mu in args.predsnrs.split(" "):
    mus.append(float(mu) - snr_offset)
...
for H in range(0, 23):
    #query = "SELECT snr FROM wspr.rx WHERE " + "rx_sign = '" + args.receiver + "' AND tx_sign = '" + args.sender + "' AND band = " + band + " AND EXTRACT(HOUR FROM time) < " + str(
    #    H) + " AND EXTRACT(HOUR FROM time) >= " + str(H - 1) + ";"
    #c.execute(query)

    data = json.load(open("/home/pavel/IdeaProjects/plotting/data/wspr/WW0WWV_EA8BFK/10/2025_01.00.json"))
    c = [entry["snr"] for entry in data if datetime.strptime(entry["time"], "%Y-%m-%d %H:%M:%S").hour == H]
...
maxval = r[0]
min = r[len(r) - 1]
linedist = stack.stackplot(x, max(count) * (stats.norm.pdf(x, mu, sigmaup) - min) / (maxval - min), color="blue",
                           alpha=0.3, lw=0)
...
maxval = r[0]
min = r[len(r) - 1]
linevoacap = stack.stackplot(x, max(count) * (stats.norm.pdf(x, mu, sigmaup) - min) / (maxval - min), color="red",
                             alpha=0.3, lw=0)
...
stack.legend(['Calculated distribution of WSPR data under median', 'Calculated distribution of WSPR data over median',
              'Raw WSPR data', 'VOACAP SNR under median distribution', 'VOACAP SNR over median distribution'])
